...
class DeepPot (DeepEval) :
    def __init__(self, 
        ...
        self.t_ntypes_spin = self.graph.get_tensor_by_name ('load/descrpt_attr/ntypes_spin:0')
        ...
        [self.ntypes, self.ntypes_spin, self.rcut, self.dfparam, self.daparam, self.tmap] = self.sess.run([self.t_ntypes, self.t_ntypes_spin, self.t_rcut, self.t_dfparam, self.t_daparam, self.t_tmap])
