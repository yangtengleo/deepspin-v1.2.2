...
class EnerFitting ():
    def __init__ (self, jdata, descrpt):
        ...
        self.use_spin = descrpt.get_use_spin()
    ...
    def _compute_output_stats(self, all_stat, rcond = 1e-3):
        ...
        energy_shift, resd, rank, s_value = np.linalg.lstsq(sys_tynatom, sys_ener, rcond = rcond)
        return energy_shift    
        ...
        for type_i in range(self.ntypes):
            ...
            if bias_atom_e is None :
                ...
                if self.use_spin[type_i]:
                    type_bias_ae = bias_atom_e[type_i] + bias_atom_e[type_i + len(self.use_spin)]
                else:
                    type_bias_ae = bias_atom_e[type_i]
            ...
            for ii in range(0, len(self.n_neuron)) :
                if ii >= 1 and self.n_neuron[ii] == self.n_neuron[ii-1] :
                    layer += one_layer( layer, 
                                        self.n_neuron[ii], 
                                        name = 'layer_'+str(ii)+'_type_'+str(type_i)+suffix, 
                                        reuse = reuse, 
                                        seed = self.seed, 
