...
class EnerSpinLoss () :
    def __init__ (self, jdata, **kwarg) :
        self.use_spin = kwarg['use_spin']
        self.starter_learning_rate = kwarg['starter_learning_rate']
        args = ClassArg()\
            .add('start_pref_e',        float,  default = 0.02)\
            .add('limit_pref_e',        float,  default = 1.00)\
            .add('start_pref_fr',       float,  default = 1000)\
            .add('limit_pref_fr',       float,  default = 1.00)\
            .add('start_pref_fm',       float,  default = 1000)\
            .add('limit_pref_fm',       float,  default = 1.00)\
            .add('start_pref_v',        float,  default = 0)\
            .add('limit_pref_v',        float,  default = 0)\
            .add('start_pref_ae',       float,  default = 0)\
            .add('limit_pref_ae',       float,  default = 0)\
        ...
        self.start_pref_e = class_data['start_pref_e']
        self.limit_pref_e = class_data['limit_pref_e']
        self.start_pref_fr = class_data['start_pref_fr']
        self.limit_pref_fr = class_data['limit_pref_fr']
        self.start_pref_fm = class_data['start_pref_fm']
        self.limit_pref_fm = class_data['limit_pref_fm']
        self.start_pref_v = class_data['start_pref_v']
        self.limit_pref_v = class_data['limit_pref_v']
        self.start_pref_ae = class_data['start_pref_ae']
        self.limit_pref_ae = class_data['limit_pref_ae']
        ...
        self.relative_f = class_data['relative_f']
        self.has_e = (self.start_pref_e != 0 or self.limit_pref_e != 0)
        self.has_fr = (self.start_pref_fr != 0 or self.limit_pref_fr != 0)
        self.has_fm = (self.start_pref_fm != 0 or self.limit_pref_fm != 0)
        self.has_v = (self.start_pref_v != 0 or self.limit_pref_v != 0)
        self.has_ae = (self.start_pref_ae != 0 or self.limit_pref_ae != 0)
        self.has_pf = (self.start_pref_pf != 0 or self.limit_pref_pf != 0)
        # data required
        add_data_requirement('energy', 1, atomic=False, must=self.has_e, high_prec=True)
        add_data_requirement('force',  3, atomic=True,  must=self.has_fr, high_prec=False)
        add_data_requirement('virial', 9, atomic=False, must=self.has_v, high_prec=False)
        add_data_requirement('atom_ener', 1, atomic=True, must=self.has_ae, high_prec=False)
        add_data_requirement('atom_pref', 1, atomic=True, must=self.has_pf, high_prec=False, repeat=3)
    ...
    def build (self, 
        ...
        force_divide_reshape = tf.reshape (force, [-1, natoms[0]*3])
        force_hat_divide_reshape = tf.reshape (force_hat, [-1, natoms[0]*3])
        force_r = tf.slice(force_divide_reshape, [0, 0], [-1, real_nloc*3])
        force_r_reshape = tf.reshape (force_r, [-1])
        force_m = tf.slice(force_divide_reshape, [0, real_nloc*3], [-1, virt_nloc*3])
        force_m_reshape = tf.reshape (force_m, [-1])
        force_r_hat = tf.slice(force_hat_divide_reshape, [0, 0], [-1, real_nloc*3])
        force_r_hat_reshape = tf.reshape (force_r_hat, [-1])
        force_m_hat = tf.slice(force_hat_divide_reshape, [0, real_nloc*3], [-1, virt_nloc*3])
        force_m_hat_reshape = tf.reshape (force_m_hat, [-1])
        l2_force_r_loss = tf.reduce_mean (tf.square(force_r_hat_reshape - force_r_reshape), name="l2_force_real_"+suffix)
        l2_force_m_loss = tf.reduce_mean (tf.square(force_m_hat_reshape - force_m_reshape), name="l2_force_mag_"+suffix)
        
        virial_reshape = tf.reshape (virial, [-1])
        virial_hat_reshape = tf.reshape (virial_hat, [-1])
        l2_virial_loss = tf.reduce_mean (tf.square(virial_hat_reshape - virial_reshape), name = "l2_virial_" + suffix)

        atom_ener_hat = tf.slice(atom_ener, [0, 0], [-1, real_nloc])
        atom_ener_reshape = tf.reshape (atom_ener, [-1])
        atom_ener_hat_reshape = tf.reshape (atom_ener_hat, [-1])
        l2_atom_ener_loss = tf.reduce_mean (tf.square(atom_ener_hat_reshape - atom_ener_reshape), name = "l2_atom_ener_" + suffix)
    ...
    def print_header(self) :
        ...
        if self.has_ae :
            print_str += prop_fmt % ('l2_ae_tst', 'l2_ae_trn')
        if self.has_fr :
            print_str += prop_fmt % ('l2_fr_tst', 'l2_fr_trn')
        if self.has_fm :
            print_str += prop_fmt % ('l2_fm_tst', 'l2_fm_trn')
        if self.has_v :
            print_str += prop_fmt % ('l2_v_tst', 'l2_v_trn')
        if self.has_pf :
            print_str += prop_fmt % ('l2_pf_tst', 'l2_pf_trn')
    ...
    def print_on_training(self, 
                          ...
                          feed_dict_test,
                          feed_dict_batch) :
        error_test, error_e_test, error_fr_test, error_fm_test, error_v_test, error_ae_test, error_pf_test \
            = sess.run([self.l2_l, \
                        self.l2_more['l2_ener_loss'], \
                        ...
                        self.l2_more['l2_pref_force_loss']],
                        feed_dict=feed_dict_test)
        error_train, error_e_train, error_fr_train, error_fm_train, error_v_train, error_ae_train, error_pf_train \
            = sess.run([self.l2_l, \
                        self.l2_more['l2_ener_loss'], \
                        self.l2_more['l2_force_r_loss'], \
                        self.l2_more['l2_force_m_loss'], \
