...
class DescrptSeA ():
    def __init__ (self, jdata):
        args = ClassArg()\
                ...
                .add('use_spin',            list,   default = None) \
                .add('spin_len',            list,   default = None) \
                .add('spin_norm',           list,   default = None)
        ...
        self.use_spin = class_data['use_spin']
        self.spin_len = class_data['spin_len']
        self.spin_norm = class_data['spin_norm']
    ...
    def get_use_spin (self) :
        return self.use_spin

    def get_spin_len (self) :
        return self.spin_len

    def get_spin_norm (self) :
    ...
    def build (self, 
        ...
        with tf.variable_scope('descrpt_attr' + suffix, reuse = reuse) :
            ...
            t_ntypes_spin = tf.constant(self.ntypes_spin, 
                                        name = 'ntypes_spin', 
                                        dtype = tf.int32)
            t_spin_len = tf.constant(self.spin_len, 
                                     name = 'spin_len', 
                                     dtype = global_tf_float_precision)
    ...
    def prod_force_virial_spin(self, atom_ener, natoms, atype) :
        [net_deriv] = tf.gradients (atom_ener, self.descrpt_reshape)
        net_deriv_reshape = tf.reshape (net_deriv, [-1, natoms[0] * self.ndescrpt])        
        force = op_module.prod_force_se_a_spin (net_deriv_reshape,
                                                self.descrpt_deriv,
                                                self.nlist,
                                                natoms,
                                                atype,
                                                ...
                                                spin_len = self.spin_len,
                                                spin_norm = self.spin_norm)
        virial, atom_virial = op_module.prod_virial_se_a(net_deriv_reshape,
                                                        self.descrpt_deriv,
                                                        self.rij,
                                                        self.nlist,
    ...
    def _compute_dstats_sys_smth(self,
                                ...
                                natoms_vec,
                                mesh) :    
        dd_all = self.sub_sess.run (self.stat_descrpt, 
                                    feed_dict = {
