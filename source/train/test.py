...
def test_ener (dp, args, append_detail = False) :
    ...
    l2va= l2v/natoms
    if dp.get_ntypes_spin() != 0:
        l2f_r = (l2err (force_r  - test_force_r))
        l2f_m = (l2err (force_m  - test_force_m))
    ...
    if dp.get_ntypes_spin() != 0:
        print("Force_real L2err     : %e eV/A" % l2f_r)
        print("Force_mag  L2err     : %e eV/A" % l2f_m)

    if detail_file is not None :
        pe = np.concatenate((np.reshape(test_data["energy"][:numb_test], [-1,1]), np.reshape(energy, [-1,1])), 
                            axis = 1)
        save_txt_file(detail_file+".e.out", pe,
                      header = '%s: data_e pred_e' % args.system,
                      append = append_detail)
        pf = np.concatenate((np.reshape(test_data["force"][:numb_test], [-1,3]), np.reshape(force, [-1,3])), 
                            axis = 1)
        save_txt_file(detail_file+".f.out", pf,
                      header = '%s: data_fx data_fy data_fz pred_fx pred_fy pred_fz' % args.system,
                      append = append_detail)
        pv = np.concatenate((np.reshape(test_data["virial"][:numb_test], [-1,9]), np.reshape(virial, [-1,9])), 
                            axis = 1)
